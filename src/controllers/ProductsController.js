...
//Na execução da tarefa conexão com o banco, essa lista mocada será removida (usando para teste)
const products = [
  {
    id: 1,
    category: 'phones',
...
const getProducts = (req, res) => {
  ...
  const totalPages = Math.ceil(productsFromCategory.length / limit);

  if (page > totalPages) {
    return res.status(404).send({ error: SystemMessages.PRODUCTS.INVALID_PAGE_NUMBER });
  }

  const paginatedProducts = productsFromCategory.slice(startIndex, endIndex);

  res.json({
    page,
    limit,
    totalPages,
    totalItems: productsFromCategory.length, // Query do banco de dados que conta a qnt total de produtos (SELECT *)
